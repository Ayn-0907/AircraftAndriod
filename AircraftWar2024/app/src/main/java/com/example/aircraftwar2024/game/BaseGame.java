...
import static com.example.aircraftwar2024.activity.GameActivity.enemyDied;
import static com.example.aircraftwar2024.activity.GameActivity.ifConnect;
...
public abstract class BaseGame extends SurfaceView implements SurfaceHolder.Callback, Runnable{
    ...
    public static boolean mbLoop; //控制绘画线程的标志位
    public static boolean mbdraw; //控制绘画线程的标志位
    ...
    public static MyMediaPlayer bgm=null;
    public static MyMediaPlayer bgm_boss=null;
    ...
    public BaseGame(Context context){
        ...
        soundPoolMap.put(1,mySoundPool.mysp.load(this.getContext(),R.raw.bomb_explosion,1));
        soundPoolMap.put(2,mySoundPool.mysp.load(this.getContext(),R.raw.bullet_hit,1));
        soundPoolMap.put(3,mySoundPool.mysp.load(this.getContext(),R.raw.game_over,1));
        soundPoolMap.put(4,mySoundPool.mysp.load(this.getContext(),R.raw.get_supply,1));
    ...
    protected List<AbstractEnemyAircraft> produceBoss() {
        ...
        if (this.getScore() >= bossScoreThreshold && !this.existBoss()) {
            ...
            if(bgm_boss!=null)bgm_boss.startMusic();
    ...
    private void crashCheckAction() throws InterruptedException {
        ...
        for (AbstractBullet bullet : enemyBullets) {
            ...
            if (heroAircraft.crash(bullet)) {
                if(isMusic){
                    mySoundPool.mysp.play(soundPoolMap.get(2),1,1,0,0,1.0f);
                }
        ...
        for (AbstractFlyingSupply flyingSupply : flyingSupplies) {
            ...
            if (heroAircraft.crash(flyingSupply) || flyingSupply.crash(heroAircraft)) {
                if(isMusic){
                    mySoundPool.mysp.play(soundPoolMap.get(4),1,1,0,0,1.0f);
                }
    ...
    private void postProcessAction() {
        for(AbstractAircraft enemyaircraft:enemyAircrafts){
            if(enemyaircraft instanceof BossEnemy&&enemyaircraft.notValid()){
                if(bgm!=null)bgm.startMusic();
                if(bgm_boss!=null)bgm_boss.stopMusic();
            }
        }
        ...
        if (heroAircraft.notValid()) {
            GameActivity.writer.println("died");
            if(!ifConnect){
                mbLoop = false;
                mbdraw = false;
                gameOverFlag = true;
            ...
            if(bgm!=null)bgm.destroyMusic();
            if(bgm_boss!=null)bgm_boss.destroyMusic();
            Log.i(TAG, "heroAircraft is not Valid");
            if(isMusic){
                mySoundPool.mysp.play(soundPoolMap.get(3),1,1,0,0,1.0f);
            }
            int scoreEnd=score;
            new Thread(){
                @Override
    ...
    private void paintScoreAndLife() {
        ...
        paintInfo.setColor(Color.argb(255,255,0,0));
        paintInfo.setTextSize(50);
        String paintScore = "SCORE:"+score;
        String paintOpScore = "ENEMY SCORE:"+enemyScore;
        String paintLife = "LIFE:"+heroAircraft.getHp();
        Log.i(TAG,heroAircraft.getHp()+"");
        canvas.drawText(paintLife,50,80,paintInfo);
        canvas.drawText(paintScore,50,130,paintInfo);
        if (ifConnect) {
            canvas.drawText(paintOpScore,50,180,paintInfo);
        }
        Log.i("paint",score+" , " + enemyScore);
    }
